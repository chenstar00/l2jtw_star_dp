...
public class AdminFightCalculator implements IAdminCommandHandler {
	...
	private void handleShow(String params, L2PcInstance activeChar) {
		...
		if (params.length() == 0) {
			...
			final var objectId1 = IdFactory.getInstance().getNextId();
			final var objectId2 = IdFactory.getInstance().getNextId();
			npc1 = new L2MonsterInstance(objectId1, NpcData.getInstance().getTemplate(mid1));
			npc2 = new L2MonsterInstance(objectId2, NpcData.getInstance().getTemplate(mid2));
		}
		...
		if (params.length() == 0) {
			...
			StringUtil.append(replyMSG, "<button value=\"Retry\" action=\"bypass -h admin_fight_calculator_show ", String.valueOf(((L2NpcTemplate) npc1.getTemplate()).getId()), " ", String
				.valueOf(((L2NpcTemplate) npc2.getTemplate()).getId()), "\"  width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
		}
